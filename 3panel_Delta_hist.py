...
for i in np.arange(len(B_spot)):
	for j in np.arange(len(D_spot)):
		 if B_spot[i] == D_spot[j]:
			Delta_Age_Hb = np.append(Delta_Age_Hb, D_Age_Hb[j] - B_Age_Hb[i])
			Spot = np.append(Spot,B_spot[i])
...
plt.xlabel('$\mu$ =%10.3f $\pm$ %10.3f\n$\sigma$ =%10.3f' % (np.mean(AGE),np.std(AGE)/(len(AGE)),np.std(AGE)),fontsize='large')

plt.subplot(1,3,2)
plt.hist(FeH)
plt.title('[Fe/H]$_{Disc}$ - [Fe/H]$_{Bulge}$')
plt.xlabel('$\mu$ =%10.3f $\pm$ %10.3f\n$\sigma$ =%10.3f' % (np.mean(FeH),np.std(FeH)/(len(FeH)),np.std(FeH)),fontsize='large')

plt.subplot(1,3,3)
plt.hist(MgFe)
plt.title('[Mg/Fe]$_{Disc}$ - [Mg/Fe]$_{Bulge}$')
plt.xlabel('$\mu$ =%10.3f $\pm$ %10.3f\n$\sigma$ =%10.3f' % (np.mean(MgFe),np.std(MgFe)/(len(MgFe)),np.std(MgFe)),fontsize='large')
plt.show()
